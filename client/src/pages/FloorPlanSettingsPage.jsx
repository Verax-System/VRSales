...
import { Button, message, Spin, Typography, Empty, Tooltip } from 'antd';
...
// Estilos embutidos para a página com o tema "Blueprint"
const PageStyles = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap');

    .floorplan-page-container {
      padding: 24px;
    ...
    .floorplan-canvas {
      ...
      border-radius: 16px;
      overflow: hidden;
      background-color: #1a202c; /* Fundo escuro tipo blueprint */
      background-image:
      ...
      <motion.div className="floorplan-page-container" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="floorplan-header">
            ...
            <Text className="header-instructions">
                Arraste as mesas para organizar o layout. As posições são salvas automaticamente ao soltar.
            </Text>
        </div>

        <motion.div className="floorplan-canvas" variants={containerVariants} initial="hidden" animate="visible">
          {tables.length > 0 ? (
            tables.map(table => (
              <Draggable
                key={table.id}
                ...
                onStart={() => handleDragStart(table.id)}
